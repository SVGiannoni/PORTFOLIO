...
- **Antes de mover el estado a `doing` o `pr_open`, asegurá la rama.** El paso 2 ya te trajo
  ...
  que hiciste. Recién después mandá el PATCH.
...
6. **Al terminar la implementación (o al abrir el Pull Request), cerrar el tramo**:
   ```bash
   cat > /tmp/cuerpo.json <<'JSON'
   ...
   listo, falta que lo mergeen", y es el último estado que un developer puede fijar.
   `merged_dev`, `in_testing`, `tested` e `in_production` los fija el repositorio y
   devuelven `400` si los mandás a mano.
7. Confirmarle al usuario qué Requerimiento quedó eligiendo, en qué rama y en qué estado
